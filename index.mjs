...
const minimumBud = stdlib.parseCurrency(2);
const Timeout = 10;
const params = { nftId, minimumBud, Timeout };
...
await ctcAuct.participants.Auctioneer({
    ...
    seeOutcome: (winner, amt) => {
        console.log(`Auctioner saw that ${stdlib.formatAddress(winner)} won with ${stdlib.formatCurrency(amt)}`);
    },
});
